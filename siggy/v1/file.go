...
func load_all(dir, prefix string) [][]byte {
  ...
  selection = filter_prefix(dirents, prefix)
  names = make([]string, len(selection))
  for i, e = range selection { names[i] = e.Name() }
  return load_all1(dir, names) }

func load_all1(dir string, names []string) [][]byte {
...
func filter_prefix(dirents []os.DirEntry, prefix string) []os.DirEntry {
