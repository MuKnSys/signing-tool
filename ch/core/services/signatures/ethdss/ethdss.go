...
type DSSArgs = struct {
	...
	Long         DistKeyShare
	Random       DistKeyShare
	H2           []int /* the QUAL set of random. paper 4.2.1. */
	Msg          *big.Int
	T            int
}
...
func (d *DSS) ProcessPartialSig(ps *PartialSig) error {
	...
	if err == nil {
		var (in_h3 bool = false; idx int)
		for _, idx = range (*d).H2 {
			if (*(*ps).Partial).I == idx {in_h3 = true; break}
		}
		if !in_h3 {
			err = errors.New("dss: partial signature from non-qualifying participant")
		}
	}
	if err == nil {
